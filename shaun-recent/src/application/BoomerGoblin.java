...
public class BoomerGoblin extends Enemy {
    ...
    // Track number of movements for auto-explosion
    private int movesCompleted;
    private int movesBeforeExplosion;
    ...
    public BoomerGoblin(int x, int y) {
        ...
        // Set random moves before explosion (3-5 moves)
        this.movesBeforeExplosion = 3 + new java.util.Random().nextInt(3); // 3, 4, or 5
    ...
    public void moveTo(int newX, int newY, String direction) {
        super.moveTo(newX, newY, direction);

        // Increment movement counter
        movesCompleted++;

        // Check if it's time to explode
        if (movesCompleted >= movesBeforeExplosion && !hasExploded) {
            // BoomerGoblins explode by taking damage equal to their HP to kill themselves
            this.takeDamage(this.getHp());
            this.hasExploded = true;
            System.out.println("[EXPLOSION] BoomerGoblin at (" + newX + "," + newY + ") exploded after " + movesCompleted + " moves!");
        }
    }
